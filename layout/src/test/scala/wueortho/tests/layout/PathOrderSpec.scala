...
class PathOrderSpec extends AnyFlatSpec, should.Matchers:
  ...
  lazy val routed =
    Routing(RoutingGraph.withoutPorts(boxes, graph, mkHorizontalPorts = true), graph, random, useCenteredRouting = true)
      .get
  ...
  "A sample with edge bundles" `should` "have segments with correct path order" in:
    ...
    routed.rightPaths(NodeIndex(54)) `shouldBe` Seq(1, 0, 4, 5)
    routed.rightPaths(NodeIndex(63)) `shouldBe` Seq(3, 1)
