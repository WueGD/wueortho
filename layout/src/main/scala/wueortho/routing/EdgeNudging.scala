...
object EdgeNudging extends NudgingCommons:
  ...
  //       )
  //       if lut.nonEmpty then // DEBUG BEGIN
  ...
  //   def segments: IndexedSeq[Segment]
  //   def eow: IndexedSeq[EndOfWorld]
  //   def obs: Seq[CNode]
  //   def isHorizontal: Boolean
  //   def allNodes: IndexedSeq[NodeData[CNode]]
  ...
  //   override lazy val obs      =
  //     obstacles.nodes.zipWithIndex.flatMap((o, i) => List(BeginObstacle(i, o.bottom), EndObstacle(i, o.top)))
  //   override lazy val allNodes = NodeData.mkNodes(segments ++ eow ++ obs, startIndex = 0)
  // end VGraph
  ...
  private class VGraph(
    ...
    override lazy val segments: IndexedSeq[CNode[Segment]] = ??? // todo make partially solved segments
