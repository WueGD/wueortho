...
object Area:
  def boundingBoxArea(boxes: VertexBoxes, routes: Seq[EdgeRoute]) =
    val r = Rect2D.boundingBoxOfRects((routes.map(e => Rect2D.boundingBox(e.points)) ++ boxes.nodes)*)
    4.0 * r.span.x1 * r.span.x2

  def convexHullArea(boxes: VertexBoxes, routes: Seq[EdgeRoute]) =
    val points = boxes.nodes.flatMap(_.corners.toList) ++ routes.flatMap(_.points)
