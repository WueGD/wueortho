...
object Crossings:
  ...
  def interEdgeDist(obs: Obstacles, routes: Seq[EdgeRoute]) =
    val (vertical, horizontal) = routes.map(separateRoute).unzip

    val hs = obs.nodes.flatMap(r => Seq(Seg(r.top, r.left, r.right), Seg(r.bottom, r.left, r.right)))
      ++ horizontal.flatten
    val vs = obs.nodes.flatMap(r => Seq(Seg(r.left, r.bottom, r.top), Seg(r.right, r.bottom, r.top)))
      ++ vertical.flatten
